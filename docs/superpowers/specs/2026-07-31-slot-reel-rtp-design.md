...
```
相異 signature 數 = 全同 5 + 僅上對 20 + 僅下對 20 + 皆不同 1 = **46**（< $5^3 = 125$）。三欄 $46^3 \approx 97{,}000$，實際受 $L_c$ 上限壓制更小。
...
**存在性**：只要 $\{w_s\}$ 同時有正有負，即必有非零非負整數解（取 $w_p > 0$、$w_q < 0$，令 $n_p = |w_q|$、$n_q = w_p$ 即得 0）。正負必同時存在，因為總有「幾乎必中」與「幾乎不中」的 signature。
...
各維 min-max 正規化。判準：新解與**現有每一組**的正規化歐氏距離皆 $\ge D$ 方可收錄。
