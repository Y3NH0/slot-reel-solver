...
# exact symbol x pattern coverage (two independent analyzers, cross-checked)
...
# is the target provably unreachable, or just not found?
...
- **Every declared symbol must appear on at least one reel.** `solver.py`'s acceptance loop filters out any candidate where `{s for reel in reels for s in reel} != set(spec.symbols)` (a symbol can otherwise go unused by pure chance, since `_run_composition`/`search_last_reel` each sample independently); `verify.py`'s `all_symbols_used` gate (fail severity, does not short-circuit) catches it in an artifact regardless of how it was produced. **Golden fixtures B and C are deliberately incomplete on this dimension** (B never uses symbols 1/4, C never uses symbol 0; only A uses all five) — this is intentional and pinned by `tests/test_verify.py`'s split "full coverage passes" (A) vs "missing symbol fails with a clear message" (B, C) tests. Do not "fix" B/C to use all symbols without updating those tests.
- **Coverage is attribution, and lives apart from the payout evaluators.** `evaluation/coverage.py` defines four nested tiers — `raw` (geometry only) ≥ `winning` (positive payout) ≥ `max_eligible` (an argmax match, ties inclusive) ≥ `unique_credit` (the sole argmax). `coverage_naive.py` (board enumeration) and `coverage_engine.py` (signature histograms) compute them along independently written match-and-attribution loops and must agree exactly; `cross_check_coverage()` raises on disagreement, because that is a program bug rather than a verdict. Raw counts are additionally asserted against `raw_match_count_factorized()`, a product of per-column cyclic-mask counts. The payout evaluators still return a scalar per spin and were not changed — do not teach them attribution. **Monte Carlo must never decide a coverage question**: a simulation cannot tell "impossible" from "rare". Enforced by AST tests in `tests/test_coverage.py`.
- **`spec.coverage` is optional and defaults to off**, so specs predating it behave identically (`each_reel_all_symbols: false`, `symbol_pattern: "none"`). When set, `verify.py` adds `per_reel_symbol_coverage` and/or `symbol_pattern_coverage` gates, and the solver enforces the bound *during generation* (`_run_composition`'s `lower_bounds`) rather than by rejecting finished candidates — rejection sampling collapses as the symbol count grows. The acceptance check is kept as an assertion of that guarantee.
- **Never report an exhausted search as an impossibility.** `solve_with_status()` returns a `SolveStatus`: `PROVEN_INFEASIBLE` (holds at every length), `BOUNDED_EXHAUSTED` (complete *within the configured length bounds* — often fixed by raising `--max-len`), `HEURISTIC_EXHAUSTED` (says nothing about existence). `solve()` still returns `ReelConfig | None` for existing callers. For homework-3x3, ideal symbol×pattern coverage is `BOUNDED_EXHAUSTED` for lengths ≤ 16 — a real exhaustive enumeration (1512 strips, 343 histogram combinations, none even reaching exact RTP), *not* a proof that it is impossible at any length.
